...
public interface PermissionRepository extends JpaRepository<Permission, Long> {
	...
	@Query("SELECT p FROM Permission p WHERE p.deleted = 0 "
			...
			+ "AND (:status IS NULL OR p.status = :status) " + "ORDER BY p.sortOrder ASC")
	Page<Permission> findByConditions(@Param("permissionName") String permissionName,
			@Param("permissionCode") String permissionCode, @Param("status") Integer status, Pageable pageable);
