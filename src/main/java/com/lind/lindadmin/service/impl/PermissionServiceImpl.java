...
public class PermissionServiceImpl implements PermissionService {
	...
	public Permission findById(Long id) {
		return permissionRepository.findById(id).filter(p -> p.getDeleted() == 0)
				.orElseThrow(() -> new BusinessException("权限不存在"));
	}

	@Override
	public PageResult<Permission> findPage(String permissionName, String permissionCode, Integer status,
		...
		Page<Permission> page = permissionRepository.findByConditions(
				...
				permissionCode != null && !permissionCode.isEmpty() ? permissionCode : null, status, pageable);
