...
public class RoleServiceImpl implements RoleService {
	...
	public Role findById(Long id) {
		return roleRepository.findById(id).filter(r -> r.getDeleted() == 0)
				.orElseThrow(() -> new BusinessException("角色不存在"));
	}

	@Override
	public PageResult<Role> findPage(String roleName, String roleCode, Integer status, Integer pageNum,
			Integer pageSize) {
		Pageable pageable = PageRequest.of(pageNum - 1, pageSize);
		Page<Role> page = roleRepository.findByConditions(roleName != null && !roleName.isEmpty() ? roleName : null,
				roleCode != null && !roleCode.isEmpty() ? roleCode : null, status, pageable);
	...
	public List<Role> findByUserId(Long userId) {
		...
		return roleRepository.findAllById(roleIds).stream().filter(r -> r.getDeleted() == 0)
